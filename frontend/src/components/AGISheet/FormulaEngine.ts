...
export class FormulaEngine {
  ...
  private initializeCustomFunctions(): void {
    ...
    this.customFunctions.set('AVERAGE', (values: number[]) => {
      ...
      return (numbers?.length ?? 0) > 0 ? numbers.reduce((a, b) => a + b) / (numbers?.length ?? 0) : 0;
    });

    this.customFunctions.set('MEDIAN', (values: number[]) => {
      const numbers = values.filter(v => typeof v === 'number').sort((a, b) => a - b);
      if ((numbers?.length ?? 0) === 0) return 0;
      const mid = Math.floor((numbers?.length ?? 0) / 2);
      return (numbers?.length ?? 0) % 2 === 0 
    ...
    this.customFunctions.set('WORDCOUNT', (text: string) => {
      return String(text).split(/\s+/).filter(word => (word?.length ?? 0) > 0).length;
    });
  ...
  private columnToIndex(column: string): number {
    let result = 0;
    for (let i = 0; i < (column?.length ?? 0); i++) {
  ...
  getDependencies(formula: string): string[] {
    ...
    while ((match = cellRefPattern.exec(cleanFormula)) !== null) {
      const colIndex = this.columnToIndex(match[1] || "A");
      const rowIndex = parseInt(match[2] || "1") - 1;
      const cellId = `${rowIndex}-${colIndex}`;
