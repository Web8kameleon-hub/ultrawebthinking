  ...
  if (isLoading) {
    return (
      <div style={{
        ...
        />
        <div style={{ marginLeft: '20px', fontSize: '18px', color: '#d4af37' } as any}>
      ...
      >
        ...
        <div style={{
          ...
          {nodes.map((node, index) => (
            ...
            >
              <div style={{ fontSize: '12px', fontWeight: 600, color: 'white' } as any}>
      ...
      >
        ...
        <div style={{ overflowX: 'auto' } as any}>
          <table style={{ width: '100%', borderCollapse: 'collapse' } as any}>
            <thead>
              <tr style={{ background: 'rgba(45, 52, 70, 0.5)' } as any}>
                <th style={{ padding: '15px', textAlign: 'left', color: '#cbd5e1', fontSize: '14px' } as any}>Node</th>
                <th style={{ padding: '15px', textAlign: 'left', color: '#cbd5e1', fontSize: '14px' } as any}>Type</th>
                <th style={{ padding: '15px', textAlign: 'left', color: '#cbd5e1', fontSize: '14px' } as any}>Activity</th>
                <th style={{ padding: '15px', textAlign: 'left', color: '#cbd5e1', fontSize: '14px' } as any}>Pulse Rate</th>
                <th style={{ padding: '15px', textAlign: 'left', color: '#cbd5e1', fontSize: '14px' } as any}>Connections</th>
                <th style={{ padding: '15px', textAlign: 'left', color: '#cbd5e1', fontSize: '14px' } as any}>Status</th>
                <th style={{ padding: '15px', textAlign: 'left', color: '#cbd5e1', fontSize: '14px' } as any}>Errors</th>
              </tr>
            </thead>
            <tbody>
              {nodes.map((node) => (
                <tr key={node.id} style={{ borderBottom: '1px solid rgba(212, 175, 55, 0.1)' } as any}>
                  <td style={{ padding: '15px', color: '#f8fafc', fontSize: '14px' } as any}>{node.name}</td>
                  <td style={{ padding: '15px', fontSize: '14px' } as any}>
                    <span style={{
                      background: getNodeColor(node.type, node.status),
                      color: 'white',
                      padding: '4px 8px',
                  ...
                  <td style={{ padding: '15px', color: '#d4af37', fontSize: '14px', fontWeight: 600 } as any}>
                    {node.activity.toFixed(1)}%
                  </td>
                  <td style={{ padding: '15px', color: '#f8fafc', fontSize: '14px' } as any}>
                  ...
                  <td style={{ padding: '15px', color: node.errorCount > 0 ? '#ef4444' : '#22c55e', fontSize: '14px' } as any}>
                    {node.errorCount}
                  </td>
                </tr>
              ))}
