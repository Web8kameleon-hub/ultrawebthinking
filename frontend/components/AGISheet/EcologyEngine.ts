...
export class EcologyEngine {
  ...
  // TypeScript utility methods for safe operations
  private safeArrayAccess<T>(array: T[], index: number, defaultValue: T): T {
    try {
      return array?.[index] ?? defaultValue;
    } catch (error) {
      console.warn(`Safe array access failed for index ${index}:`, error);
      return defaultValue;
    }
  }

  private validateSpecimens(specimens: SpecimenData[]): boolean {
    return Array.isArray(specimens) && specimens.length > 0 && specimens.every(s => s?.id && s?.species);
  }

  private handleError<T>(operation: string, error: unknown, fallback: T): T {
    console.error(`EcologyEngine error in ${operation}:`, error);
    return fallback;
  }
  ...
  public async assessConservationStatus(specimens: SpecimenData[], ecosystemMetrics: EcosystemMetrics): Promise<EcologyAnalysisResult> {
    try {
      ...
      if (!this.validateSpecimens(specimens)) {
        throw new Error('Invalid specimens data provided');
      }
      
      if (!ecosystemMetrics || typeof ecosystemMetrics.biodiversityIndex !== 'number') {
        throw new Error('Invalid ecosystem metrics provided');
      }
      ...
      return this.handleError('assessConservationStatus', error, this.getDefaultAnalysisResult());
    }
  }

  private getDefaultAnalysisResult(): EcologyAnalysisResult {
    return {
      conservationStatus: {
        overallAssessment: 'unknown' as any,
        redListCriteria: [],
      ...
      connectivityAnalysis: {
        ...
        corridorEffectiveness: 0,
        barrierIdentification: [],
        connectivitySolutions: [],
        landscapePermeability: 0
      }
    };
  }
  ...
  private analyzeFoodWebPosition(specimens: SpecimenData[]) {
    try {
      // Safe array access with proper TypeScript checking
      const specimen = specimens?.[0];
      if (!specimen || specimens.length === 0) {
        return {
          trophicLevel: 1,
      ...
      console.error('Error analyzing food web position:', error);
      return {
        trophicLevel: 1,
        role: 'producer' as const,
        keyInteractions: [],
  ...
  private generateKeyInteractions(specimens: SpecimenData[]): Array<{
    partner: string;
    relationship: 'predator' | 'prey' | 'competitor' | 'mutualist' | 'parasite';
    strength: number;
  }> {
    ...
    try {
      if (!specimens || specimens.length === 0) return interactions;
      
      const firstSpecimen = specimens[0];
      if (!firstSpecimen) return interactions;
      
      // TypeScript-safe loop with proper bounds checking
      const maxInteractions = Math.min(3, specimens.length - 1);
      for (let i = 0; i < maxInteractions; i++) {
        const nextSpecimen = specimens[i + 1];
        if (!nextSpecimen?.species) continue; // Safe property access
        
        const relationship = this.determineRelationship(firstSpecimen, nextSpecimen);
        const validRelationships: Array<'predator' | 'prey' | 'competitor' | 'mutualist' | 'parasite'> = 
          ['predator', 'prey', 'competitor', 'mutualist', 'parasite'];
        
        const typedRelationship = validRelationships.includes(relationship as any) 
          ? relationship as 'predator' | 'prey' | 'competitor' | 'mutualist' | 'parasite'
          : 'competitor'; // Default fallback
        
        interactions.push({
          partner: nextSpecimen.species,
      ...
      console.error('Error generating key interactions:', error);
      return interactions; // Return empty array on error
    }
  }
  ...
  private determineThreatSeverity(threatType: string, specimens: SpecimenData[], ecosystemMetrics: EcosystemMetrics): string {
    try {
      ...
      const severityLevels: string[] = ['low', 'medium', 'high'];
      const randomIndex = Math.floor(Math.random() * severityLevels.length);
      const selectedSeverity = severityLevels[randomIndex];
      
      return selectedSeverity ?? 'medium'; // TypeScript null coalescing
    } catch (error) {
      console.error('Error determining threat severity:', error);
