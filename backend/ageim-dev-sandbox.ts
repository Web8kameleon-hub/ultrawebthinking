...
export class AGEIMDevSandbox {
  ...
  private async generateFixes(tscOutput: string) {
    ...
    if (tscOutput.includes("Cannot find module")) {
      fixes.push({
        description: "Create missing module defaultValues",
        apply: async () => {
          // Extract missing modules and create defaultValues
          ...
          for (const match of moduleMatches) {
            ...
            if (modulePath && !modulePath.startsWith('node_modules')) {
              await this.createModuledefaultValue(modulePath)
            }
          }
        }
      })
  ...
  private async createModuledefaultValue(modulePath: string) {
...
export const ${componentName} = () => {
  return (
    <div>
      <h2>${componentName}</h2>
      <p>defaultValue component generated by AGEIM</p>
    </div>
  );
};
...
`
    ...
    this.log('info', `📄 Created defaultValue: ${fullPath}`)
  }
