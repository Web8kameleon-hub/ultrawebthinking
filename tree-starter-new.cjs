...
class ASITreeManager {
    ...
    async plantTree() {
        ...
        try {
            ...
            await this.growBranches(config.tree.branches.components);
    ...
    async plantRoots(rootsConfig) {
        ...
        const registryProcess = spawn('node', ['registry/registry-service.cjs'], {
            cwd: __dirname,
    ...
    async growBranches(branchesConfig) {
        ...
        for (const branch of branchesConfig) {
            ...
            await this.growFruit(branch, { name: 'services' });
        }
    }
    ...
    async growFruit(fruit, branch) {
        ...
        try {
            ...
            const scriptPath = fruit.command.startsWith('modules/') ? 
                fruit.command.replace('.js', '.cjs') : `modules/${fruit.command.replace('.js', '.cjs')}`;
