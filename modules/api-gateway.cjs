...
// API Gateway Module - NO PORT MODE (Integrated into Ultra SaaS Dashboard)
...
class ApiGatewayModule {
    ...
    setupRoutes() {
        ...
        // API proxy routes - using middleware approach
        this.app.use('/api/:serviceName', async (req, res) => {
            const serviceName = req.params.serviceName;
            const apiPath = req.url.substring(1); // Remove leading slash
        ...
        this.app.use('/proxy/:port', async (req, res) => {
            ...
            const apiPath = pathParts.slice(1).join('/'); // Remove the first empty part
